...
ggplot(predicted %>% filter(measure == "Target of intervention"), 
       aes(x = condition, fill = factor(response, levels = c("Internal", "Structural", "Other")))) +
...
ggplot(data %>% filter(measure == "Possibility of change"), 
...
ggplot(data %>% filter(measure == "Normativity"), 
       aes(x = condition, y = response)) +
  ggtitle("Normativity") +
  scale_y_continuous(breaks = c(1, 2, 3, 4, 5, 6),
                     labels = c("Very, very bad: 1", "Pretty bad: 2", "A little bad: 3", "A little good: 4", "Pretty good: 5", "Very, very good: 6")) +
...
# Plot: target of intervention
ggplot(data %>% filter(measure == "Target of intervention"), 
       aes(x = condition, fill = factor(response, levels = c("Internal", "Structural", "Other")))) +
  geom_bar(position = "fill") +
  guides(fill = guide_legend(title = "Target of intervention")) +
  scale_y_continuous(name = "Percentage of participants") +
  ggtitle("Target of intervention") 
ggsave("intervention.png")
