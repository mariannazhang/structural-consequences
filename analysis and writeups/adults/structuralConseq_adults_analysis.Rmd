...
## Power analysis from pilot data
...
```{r}
# Calculate inter-rater reliability for intervention coding
data %>% 
  mutate(intervention_coded_same = (intervention_coded_1 == intervention_coded_2)) %>% 
  summarize(intervention_percent_agreement = sum(intervention_coded_same)/n(intervention_coded_same))

cohen.kappa(data %>% select(intervention_coded_1, intervention_coded_2))
```
...
data.intervention$intervention_coded <- data_intervention$intervention_coded %>% 
  str_replace_all(" ", "_") %>% 
  str_replace_all("/", "_") %>% 
  str_replace_all("'", "") 
