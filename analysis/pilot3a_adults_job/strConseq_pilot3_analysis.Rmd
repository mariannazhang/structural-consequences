...
data_raw <- read_csv(here("data", "strConseq_data_pilot3a_coded.csv"))
```
...
data <- data_raw %>%
  ...
  # Rename column typos
  rename(check_group_job = check_group_berry,
         check_fruit = check_berry) %>% 
  ...
  mutate(condition = condition %>% 
         ...
         check_group_job = check_group_job %>% str_replace_all("\n", ""),
         ...
         switched_context = switched_context %>% recode("For sure aquafruit farmer" = 1,
                                                        "Maybe aquafruit farmer" = 2,
                                                        "Maybe terrafruit farmer" = 3,
                                                        "For sure terrafruit farmer" = 4),
...
sample <- data %>% 
  summarize(check_group_job = sum(check_group_job != "Vawns"),
            check_group = sum(check_group != "Zarpies,Vawns"),
            check_fruit = sum(check_fruit != "Aquafruit,Terrafruit"), 
...
data <- data %>%
  filter(check_group_job == "Vawns" &
           check_group == "Zarpies,Vawns" &
           check_fruit == "Aquafruit,Terrafruit" &
...
ggplot(data, aes(x = condition, y = possibility_change)) +
  ...
  scale_y_continuous(name = "Can Zarpies also work as terrafruit farmers?",
                     breaks = c(1, 2, 3, 4),
...
ggplot(data, aes(x = condition, y = switched_context)) +
  ...
  scale_y_continuous(name = "a group member outside context",
                     breaks = c(1, 2, 3, 4),
                     labels = c("for sure aquafruit farmer", "maybe aquafruit farmer", "maybe terrafruit farmer", "for sure terrafruit farmer")) +
  geom_hline(yintercept = mean(c(1, 2, 3, 4)), linetype = "dashed") +
  geom_point(position = position_jitter(width = 0.2, height = 0.1),
             size = 2, alpha = 0.1) + 
  stat_summary(fun.data = "mean_cl_boot",
