...
data_raw <- read_excel(here("data", "strConseq_data_pilot2.xlsx"), sheet = 1)
```
...
# Power analysis for actual study
...
contrasts <- lm(possibility_change ~ condition + intervention_diff, 
  ...
  pairs() %>% 
  summary() %>% 
  pull(contrast) # get the names of each pairwise condition comparison

# set up simulation grid
power <- crossing(
  alpha = alpha,
  sample_size = c(20, 22, 25, 30, 35), # test these sample sizes
...
bootstrap_analysis <- function(sample_size, alpha = 0.05, simulation = NULL){
  ...
  # set up dummy columns for target of intervention responses
  resample <- resample %>% 
    mutate(
    # Cleaning
    intervention_coded = intervention_coded %>% 
      ...
      str_replace_all("/", "_") %>% 
      str_replace_all("'", "") %>% 
      factor(levels = intervention_codes_clean),
    # Spread using dummy variables
    counts = 1) %>% 
    # Add any missing intervention code values, keeping all other columns the same (workaround for spread(drop = FALSE))
    complete(nesting(!!!select(., participant:switched_context, intervention_diff)), intervention_coded) %>% 
    # Spread
  ...
  results <- tibble(
    ...
    modify_Zarpies_beliefs_culture = brglm(modify_Zarpies_beliefs_culture ~ condition + intervention_diff,
      ...
      summary() %>% 
      pull(p.value),
    modify_environment = brglm(modify_environment ~ condition + intervention_diff,
                                 family = "binomial",
                                 data = resample) %>% 
      emmeans("condition") %>% 
      pairs(adjust = "fdr") %>% 
...
# we want to find a sample size that gives us at least 80% power on smallest contrast of interest
power_results <- power_exp %>% 
  ...
  summarize(possibility_change = sum(possibility_change < .05) / n(),
            ...
            modify_environment = sum(modify_environment < .05) / n())
power_results
write.csv(power_results, file = "power_results.csv") # output to .csv file
