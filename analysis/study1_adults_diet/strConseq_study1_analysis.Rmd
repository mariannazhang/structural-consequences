...
intervention_colors = c("modify Zarpies biology" = "#D9695F",
                        "change group biology" = "#D9695F",
                        
                        "modify Zarpies beliefs/culture" = "#F2BF80",
                        "change group beliefs/culture" = "#F2BF80",
                        
                        "acquire okinberries" = "#49808C",
                        "change environment \n (increase food access)" = "#49808C",
                        
                        "modify okinberries" = "#D9BAB7",
                        "change the food" = "#D9BAB7",
                        
                        # "acquire pogoberries" = "#C3C7B6",
                        "acquire pogoberries" = "#D9D9D9",
                        "other" = "#D9D9D9",
                        "ambiguous" = "#D9D9D9",
                        "don't know" = "#D9D9D9"
                        # "don't know" = "#9E9E9E"
...
intervention_colors_highlight = c("modify Zarpies biology" = "#D9BAB7",
                        "change group biology" = "#D9BAB7",
                        
                        "modify Zarpies beliefs/culture" = "#D9BAB7",
                        "change group beliefs/culture" = "#D9BAB7",
                        
                        "acquire okinberries" = "#49808C",
                        "change environment \n (increase food access)" = "#49808C",
                        
                        "modify okinberries" = "#D9D9D9",
                        "change the food" = "#D9D9D9",
...
ggplot(data, aes(x = condition, y = possibility_change)) +
  ...
  scale_y_continuous(name = "",
                     # name = "Can Zarpies also eat okinberries?",
                     breaks = c(1, 2, 3, 4),
...
# Plot responses: normativity
ggplot(data, aes(x = condition, y = normativity)) +
  ggtitle("Structural representations lead to more positive attitudes \nabout group members who are different") +
  theme(plot.title.position = "plot", 
        plot.title = element_text(hjust = 0.5)) +
  scale_y_reverse(name = "a group member with a different diet",
                     breaks = c(1, 2, 3, 4, 5, 6),
                     labels = c("very, very good", "pretty good", "a little good", "a little bad", "pretty bad", "very, very bad")) +
  geom_hline(yintercept = mean(c(1, 2, 3, 4, 5, 6)), linetype = "dashed") +
  geom_point(position = position_jitter(width = 0.2, height = 0.1),
  ...
  stat_summary(fun = "mean",
               ...
               fill = "black",
               size = 5)
ggsave("normativity_simple.png", height = 5, width = 7)
...
# Plot responses: switched context
ggplot(data, aes(x = condition, y = switched_context)) +
  ggtitle("Structural representations lead to reluctance to generalize \n beyond the group's structural context") +
  theme(plot.title.position = "plot", 
        plot.title = element_text(hjust = 0.5)) +
  ...
  stat_summary(fun = "mean",
               ...
               fill = "black",
               size = 5)
ggsave("switched_context_simple.png", width = 7)
...
ggplot(data_tidy_intervention, 
       aes(x = condition, fill = intervention_coded %>% 
             recode("modify Zarpies biology" = "change group biology",
                    "modify Zarpies beliefs/culture" = "change group beliefs/culture",
                    "acquire okinberries" = "change environment \n (increase food access)",
                    "modify okinberries" = "change the food",
                    "acquire pogoberries" = "other",
                    "ambiguous" = "other",
                    "other" = "other",
                    "don't know" = "other"))) +
  # ggtitle("Structural representations suggest intervening on \nstructural factors (e.g. lack of food access), rather than the group") +
  geom_bar(position = "fill") +
  scale_fill_manual(values = intervention_colors) +
  guides(fill = guide_legend(title = NULL)) +
  scale_y_continuous(name = "",
    # name = "What would you do, so that \n Zarpies can eat okinberries?",
                     ...
                     expand = c(0,0)) +
  scale_x_discrete(expand = c(0,0))
ggsave("intervention_simple.png", width = 10)
...
ggplot(data_tidy_intervention, 
  ...
  guides(fill = guide_legend(title = NULL)) +
  scale_y_continuous(name = "",
                     # name = "How difficult would it be?",
                     labels = c("very easy", "somewhat easy", "somewhat difficult", "very difficult")) 
...
# odds ratios?
exp(cbind(coef(acquire_okinberries_glm), confint(acquire_okinberries_glm)))  

# more than half in structural condition? 
binom.test(x = data_intervention %>% filter(condition == "structural") %>% pull(acquire_okinberries) %>% sum(),
           n = data_intervention %>% filter(condition == "structural") %>% pull(acquire_okinberries) %>% length(),
           p = 0.5, # compare against half
           alternative = "two.sided")
```
...
## Difficulty of intervention
```{r check-diff-intervention}
# Intervention difficulty: linear model, controlling for difficulty of intervention
...
cor_measures <- cor(data %>% select(switched_context, normativity, intervention_diff))

cor_measures_sig <- cor.mtest(data %>% select(switched_context, normativity, intervention_diff), 
                              conf.level = 0.95)
