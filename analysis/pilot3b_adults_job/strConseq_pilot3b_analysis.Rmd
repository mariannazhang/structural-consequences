...
data_raw <- read_csv(here("data", "strConseq_data_pilot3b_coded.csv"))
```
...
sample <- data %>% 
  summarize(check_group_job = sum(check_group_job != "Vawns"),
            ...
            check_job = sum(check_job != "Aquafruit farmer,Grocer"), 
...
data <- data %>%
  filter(check_group_job == "Vawns" &
           ...
           check_job == "Aquafruit farmer,Grocer" &
...
ggplot(data, aes(x = condition, y = possibility_change)) +
  ...
  scale_y_continuous(name = "Could Zarpies also work as grocers?",
                     breaks = c(1, 2, 3, 4),
...
ggplot(data, aes(x = condition, y = switched_context)) +
  ...
  scale_y_continuous(name = "a group member outside context",
                     breaks = c(1, 2, 3, 4),
                     labels = c("for sure aquafruit farmer", "maybe aquafruit farmer", "maybe grocer", "for sure grocer")) +
  geom_hline(yintercept = mean(c(1, 2, 3, 4)), linetype = "dashed") +
  geom_point(position = position_jitter(width = 0.2, height = 0.1),
             size = 2, alpha = 0.1) + 
  stat_summary(fun.data = "mean_cl_boot",
